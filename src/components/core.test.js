...
describe("core functions", function () {
  ...
  describe("Remove tree node", function () {
    function r(before, after, key) {
      const treeBefore = schema2tree("root", before.schema, before.uiSchema);
      const treeAfter = removeNodeByPath(treeBefore, key.split("."));
  ...
  describe("add tree node", function () {
    function r(before, after, key, position, add, name) {
      const treeBefore = schema2tree("root", before.schema, before.uiSchema);
      const nodeAdd = schema2tree(name, add.schema, add.uiSchema)[0];
    ...
    it("brother nodes should not have the same names", function () {
      ...
      const newTree = addNodeByPath(tree, "root".split("."), 0, {
        name: "foo",
        schema: { type: "string" },
      });
  ...
  describe("update tree node", function () {
    ...
    it("update root name", function () {
      ...
      const treeBefore = schema2tree("root_before", data.schema);
      const expectedTreeAfter = schema2tree("root_after", data.schema);
      const treeAfter = updateNodeByPath(treeBefore, ["root_before"], {
        name: "root_after",
      });
      expect(treeAfter).toEqual(expectedTreeAfter);
    });
