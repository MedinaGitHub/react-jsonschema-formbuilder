...
function updateArrayIndex(tree) {
  return tree.map((node, i) =>
    node.name === i.toString()
      ? node
      : updateNodeParentKeyAndName(node, null, i.toString())
  );
}
  ...
  if (!tail.length) {
    ...
    for (const i in tree) {
      ...
      if (position < 0) {
        ...
        newTree.push(
          updateNodeParentKeyAndName(node2Add, getNodeParentKey(cn))
        );
      }
  ...
  for (const node of tree) {
    ...
    if (!tail.length) {
      ...
      if (schema) {
        ...
        if (!newSchema.type) {
          ...
          if (newSchema.type !== oldSchema.type) {
            if (
              allowedUpdatedTypes.includes(newSchema.type) &&
      ...
      if (node.children) {
        ...
        updated = true;
        let newNode = updateNodeByNewChildren(node, newChildren); // update required
        ...
        if (
          ...
          newNode.schema = Object.assign({}, newSchema, {
            required: newSchema.required.map((a) =>
              a === tail[0] ? nodeUpdate.name : a
            ),
          });
        }
      ...
      if (node.schema.type === "array") {
        ...
        if (Array.isArray(node.schema.items)) {
          if (+index < node.schema.items.length) {
            ...
            return rest
              ? n && getNodeByRjsfId(n.children, rjsfId.slice(i + 1))
              : n;
          } else {
